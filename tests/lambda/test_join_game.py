...
from lambdas.join_game import lambda_handler
...
def websocket_comm_service_class(mocker, comm_service):
    mocker.patch('lambdas.join_game.WebSocketCommService', return_value=comm_service)
...
def state_machine_class(mocker, state_machine):
    mocker.patch('lambdas.join_game.StateMachine', return_value=state_machine)
...
def test_lambda_handler(mocker, event, repository, comm_service, state_machine):
    # Given
    mocked_uuid = mocker.patch("lambdas.join_game.uuid")
